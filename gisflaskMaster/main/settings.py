# -*- coding: utf-8 -*-
"""
    不同的环境（比如开发环境和生产环境）使用不同的配置
    总体意思是创建基类Config来保存通用配置 其他的各环境使用不同的配置 再用一个字典提供选择
"""
import os
import sys

basedir = os.path.dirname(__file__)  # 输出该脚本所在的完整路径
...
class DevelopmentConfig(BaseConfig):
    SQLALCHEMY_DATABASE_URI = "postgresql://postgres:861276@localhost:5432/ding"  # postgresql(安装了postgis扩展)的连接字符串，例如：postgresql://(user):(password)@(ip):(port)/(database)
