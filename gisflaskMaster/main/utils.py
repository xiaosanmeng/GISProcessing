...
def initLog():
    ...
    final_log_file = os.path.join('logging', '{0}.log'.format(currtime))
...
class Geoserver:
    ...
    geocat = Catalog(geourl, username=app.config['SERVERNAME'],
                     password=app.config['SERVERPWD'])  # create a Catalog object
    ...
    def createCoverageStoreUrl(self, name, filePath, workSpaceName):
        try:
            ...
            url2 = "{0}gwc/service/wmts?layer={3}%3A{1}&style=&tilematrixset=EPSG%3A900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=image%2Fpng&{2}".format(
                app.config['SERVERURL'], name, "TileMatrix=EPSG%3A900913%3A{z}&TileCol={x}&TileRow={y}", workSpaceName)
